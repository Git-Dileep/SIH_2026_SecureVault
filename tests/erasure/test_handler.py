"""
Tests for erasure.handler — Owner: Person 5

Covers:
  - Authorization (valid key, invalid key, env-var override)
  - Clear operation (authorized, unauthorized, missing file)
  - Purge operation (authorized, unauthorized, missing file)
  - Audit trail integration (entries recorded for every step)
"""
...
def temp_file():
    ...
    fd, path = tempfile.mkstemp(prefix="erasure_test_", suffix=".dat")
    content = b"SecureVault test data - this content should be erased.\n" * 100
    os.write(fd, content)
    os.close(fd)
    yield path
